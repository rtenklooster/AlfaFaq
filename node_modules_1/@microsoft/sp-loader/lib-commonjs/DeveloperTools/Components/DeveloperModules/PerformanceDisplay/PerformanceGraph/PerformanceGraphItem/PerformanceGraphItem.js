...
var PerformanceGraphItem = /** @class */ (function (_super) {
    ...
    PerformanceGraphItem.prototype.render = function () {
        ...
        if (perfItem.breakdown) {
             ...
             */
            graphBar.push(this._drawGraphBar(perfItem.breakdown.render.startVal, perfItem.breakdown.render.duration, PerformanceLegend_module_scss_1.default.msBlueMid, "".concat(strings.RenderTimeLegendLabel, " ").concat(perfItem.breakdown.render.duration, " ms")));
            if (perfItem.breakdown.modulesLoaded) {
                graphBar.push(this._drawGraphBar(perfItem.breakdown.modulesLoaded.startVal, perfItem.breakdown.modulesLoaded.duration, PerformanceLegend_module_scss_1.default.msBlue, "".concat(strings.ModuleLoadingTimeLabel, " ").concat(perfItem.breakdown.modulesLoaded.duration, " ms")));
            }
            if (perfItem.breakdown.lazyLoading) {
                graphBar.push(this._drawGraphBar(perfItem.breakdown.lazyLoading.startVal, perfItem.breakdown.lazyLoading.duration, PerformanceLegend_module_scss_1.default.msLightGreen, "".concat(strings.ModuleLazyLoadingDelayLabel, " ").concat(perfItem.breakdown.lazyLoading.duration, " ms")));
            }
            if (perfItem.breakdown.init) {
                graphBar.push(this._drawGraphBar(perfItem.breakdown.init.startVal, perfItem.breakdown.init.duration, PerformanceLegend_module_scss_1.default.msPurpleLight, "".concat(strings.InitializationTimeLabel, " ").concat(perfItem.breakdown.init.duration, " ms")));
            }
            if (perfItem.breakdown.dataFetch) {
                for (var _i = 0, _a = perfItem.breakdown.dataFetch; _i < _a.length; _i++) {
                    var apiCall = _a[_i];
                    graphBar.push(this._drawGraphBar(apiCall.startVal, apiCall.duration, PerformanceLegend_module_scss_1.default.msTealLight, "".concat(strings.DataFetchTimeLabel, " ").concat(apiCall.duration, " ms")));
                }
            }
        }
        else {
